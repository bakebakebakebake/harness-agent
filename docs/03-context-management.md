...
### 当前 Harness-Agent 的落地做法

在 Harness-Agent 当前实现里,这个思路已经进一步具体化为三层:

- **transcript 证据层**:`~/.harness-agent/memory/transcripts/<session-id>.jsonl`
- **Markdown 记忆卡**:`<workdir>/.agents/memory/project/*.md` 与 `~/.harness-agent/memory/user/*.md`
- **SQLite 索引层**:`~/.harness-agent/memory/index.sqlite`
